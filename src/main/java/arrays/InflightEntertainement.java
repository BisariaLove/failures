package arrays;
/*
 * @author love.bisaria on 16/09/18
 */
...
public class InflightEntertainement {


    public static boolean canTwoMoviesFillFlight(int[] movieLengths, int flightLength) {


        Set<Integer> movieLengthSet = new HashSet();

        for(int i=0; i< movieLengths.length; i++){
            movieLengthSet.add(movieLengths[i]);
        }

        for(Integer time : movieLengths){
            if(time>flightLength){
                continue;
            }

            movieLengthSet.remove(time);
            if(movieLengthSet.contains(flightLength-time)){
                return true;
            }


            movieLengthSet.add(time);
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(InflightEntertainement.class);
